...
const nameValidator = (fieldName: string) =>
  z
    ...
    .max(40, `${fieldName} не може перевищувати 40 символів`)
...
export const UserProfileSchema = z.object({
  ...
  email: z.email({ message: "Введіть дійсну електронну адресу" }).trim(),
  // linkedin_url: optionalUrlValidator("LinkedIn профіль", /^https?:\/\/(www\.)?linkedin\.com\/.*$/),
  // cv: optionalUrlValidator("CV", undefined, ["pdf", "doc", "docx"]),
});
